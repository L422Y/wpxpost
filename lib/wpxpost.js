...
let wpxpost = {
    ...
    createPost: async (info) => {
        ...
        const author = wpxpost.wpusers.find(f => f.name === info['author'])
        if (author) {
            postObject.author = author.id
        } else if (wpxpost.opts.authorID) {
            postObject.author = authorID
        }

        await wpxpost.wp.posts().create(postObject)
            .then(async (post) => {
                let filename = 'images/' + info.heroImage.split('/').pop()
                await downloadImage(info.heroImage, filename)
                    .then(async () => {
                        await wpxpost.wp.media().file(filename).create({
                            title: info.title,
                            post: post.id
                        }).then(async (media) => {
                            await wpxpost.wp.posts().id(post.id).update({
